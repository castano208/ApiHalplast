...
const Tipo = require('../modules/tipoPqrs');
...
const tipoPqrsPut = async (req, res = response) => {
    try {
        ...
        const tipo = await Tipo.findOneAndUpdate({ _id: id_tipo }, { nombre, motivos }, { new: true });
...
const tipoPqrsDelete = async (req, res = response) => {
    try {
        const { id_tipo } = req.params;
        const tipo = await Tipo.findOneAndDelete({ _id: id_tipo });
